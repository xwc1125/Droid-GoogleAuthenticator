 ...
 */
public class TimeotpCountdownTask implements Runnable {
    private final TimeotpCounter mCounter;
    private final TimeotpClock mClock;
     ...
     *                                        value.
     */
    public TimeotpCountdownTask(TimeotpCounter counter, TimeotpClock clock, long remainingTimeNotificationPeriod) {
        mCounter = counter;
        mClock = clock;
    ...
    public void run() {
        ...
        long now = mClock.currentTimeMillis();//当前时间
        long counterValue = getCounterValue(now);//当前步数
