 ...
 */
public class TimeotpCounter {

    private static TimeotpCounter sTimeotpCounter;

    public static synchronized TimeotpCounter getInstance(long timeStep) {
        if (sTimeotpCounter == null) {//初始化TotpClock
            sTimeotpCounter = new TimeotpCounter(timeStep);
        }
        return sTimeotpCounter;
    }
     ...
     */
    public TimeotpCounter(long timeStep) {
        this(timeStep, 0);
    }
     ...
     */
    public TimeotpCounter(long timeStep, long startTime) {
        if (timeStep < 1) {//时间步数；起始时间
            throw new IllegalArgumentException("Time step must be positive: " + timeStep);
        }
        assertValidTime(startTime);//判断起始时间是否可用
