 ...
 */
public class TimeotpClock implements SharedPreferences.OnSharedPreferenceChangeListener {

    private static TimeotpClock sTimeotpClock;

    public static synchronized TimeotpClock getInstance(Context context) {
        if (sTimeotpClock == null) {//初始化TotpClock
            sTimeotpClock = new TimeotpClock(context);
        }
        return sTimeotpClock;
    }

    // @VisibleForTesting
    public static final String PREFERENCE_KEY_OFFSET_MINUTES = "timeCorrectionMinutes";
    ...
    public TimeotpClock(Context context) {
